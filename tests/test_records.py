...
from pbh.records import FORMAT, VERSION, read_initial, read_record, write_initial, write_record
...
def test_the_initial_file_round_trips_the_state_the_grid_the_time_and_the_provenance(tmp_path: Path, m: Map):
    ...
    write_initial(path, state, geo, 0.0, provenance)
    record = read_initial(path)
    assert np.array_equal(record.state.E, state.E)
    assert np.array_equal(record.state.U, state.U)
...
def test_a_record_in_any_group_is_the_same_record_so_checkpoints_share_it(tmp_path: Path):
    ...
    with h5py.File(path, "w") as f:
        write_record(h5.create_group(f, "checkpoints/000012"), excised, geo.X[: geo.N + 1], 3.5, 4, {"step": 12})
...
def test_the_provenance_accepts_numpy_values_and_refuses_what_json_cannot_hold(tmp_path: Path):
    ...
    write_initial(
        path, state, geo, 0.0, {"ratio": np.float64(0.036), "N": np.int64(60), "fractions": np.array([1e-9, 2e-8])}
    )
    record = read_initial(path)
    ...
    with pytest.raises(TypeError, match="not JSON-serialisable"):
        write_initial(tmp_path / "bad.initial.h5", state, geo, 0.0, {"geometry": geo})
...
def test_a_state_that_does_not_fit_the_grid_and_a_foreign_file_are_refused(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="does not fit the grid"):
        write_initial(tmp_path / "x.h5", state, other, 0.0, {})
