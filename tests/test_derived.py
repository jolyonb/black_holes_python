...
def setup(m: Map, N: int, x_max: float, xi: float = 0.6, j_e: int = 0) -> tuple[Geometry, Background, StencilWeights]:
...
def test_on_frw_every_derived_field_has_its_frw_value(m: Map, j_e: int):
    geo, bg, w = setup(m, 40, 6.0, j_e=j_e)
...
def test_the_innermost_face_mass_is_the_innermost_density_identically():
    ...
    geo, bg, w = setup(SinhStretch(scale=1.0), 12, 3.0)
    s = frw_state(geo)
    E = s.E.copy()
    E[0] *= 1.37
