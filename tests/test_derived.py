...
def setup(m: Map, N: int, x_max: float, xi: float = 0.6, j_e: int = 0) -> tuple[Geometry, Background, Layout]:
    return Geometry.of(*m.at(xi, face_labels(N, x_max))), Background.at(EOS, xi), Layout(N, j_e)
...
def test_on_frw_every_derived_field_has_its_frw_value(m: Map, j_e: int):
    geo, bg, lay = setup(m, 40, 6.0, j_e=j_e)
    d = derive(frw_state(geo, j_e), geo, bg, EOS, lay)
    cells, faces = lay.cells, lay.faces
...
def test_entries_below_the_excision_face_are_nan_and_the_origin_mt_is_nan():
    geo, bg, lay = setup(IdentityMap(), 10, 2.0, j_e=3)
    d = derive(frw_state(geo, 3), geo, bg, EOS, lay)
    for name in ("rho", "ephi"):
        assert np.all(np.isnan(getattr(d, name)[:3]))
        assert np.all(np.isfinite(getattr(d, name)[3:]))
    for name in ("M", "mt", "Gammabar2"):
        assert np.all(np.isnan(getattr(d, name)[:3]))
        assert np.all(np.isfinite(getattr(d, name)[3:]))
...
def test_the_innermost_face_mass_is_the_innermost_density_identically():
    ...
    geo, bg, lay = setup(SinhStretch(scale=1.0), 12, 3.0)
    s = frw_state(geo)
    E = s.E.copy()
    E[0] *= 1.37
    d = derive(State(E=E, U=s.U, W=0.0), geo, bg, EOS, lay)
    assert d.mt[1] == pytest.approx(d.rho[0], rel=1e-15)


@pytest.mark.parametrize("m", MAPS)
def test_exact_contents_of_an_even_density_give_the_printed_mass_exactly(m: Map):
    ...
    E = np.diff(antiderivative[: lay.N + 1])
    d = derive(State(E=E, U=geo.X.copy(), W=0.0), geo, bg, EOS, lay)
    X_faces = geo.X[1:]
...
def test_the_cumulative_sum_is_accurate_to_round_off_at_large_n():
    ...
    geo, bg, lay = setup(IdentityMap(), 4000, 20.0)
    rng = np.random.default_rng(7)
    E = geo.dV * rng.uniform(0.999, 1.001, size=lay.N)  # near FRW, so Gammabar^2 stays positive at X = 20
    d = derive(State(E=E, U=geo.X.copy(), W=0.0), geo, bg, EOS, lay)
    exact = np.array([3.0 * math.fsum(E[:j]) for j in range(1, lay.N + 1)])
...
def test_after_excision_the_mass_starts_at_m_e_and_sums_only_retained_cells():
    geo, bg, lay = setup(IdentityMap(), 10, 2.0, j_e=4)
    s = frw_state(geo, 4)
    M_e = 2.5 * s.M_e
    d = derive(State(E=s.E, U=s.U, W=0.0, M_e=M_e), geo, bg, EOS, lay)
    assert d.M[4] == M_e
...
def test_a_non_positive_density_aborts_naming_the_cell():
    geo, bg, lay = setup(IdentityMap(), 10, 2.0)
    s = frw_state(geo)
    E = s.E.copy()
    E[6] = -1e-3
    with pytest.raises(NotHyperbolicError, match=r"rho\[6\]") as info:
        derive(State(E=E, U=s.U, W=0.0), geo, bg, EOS, lay)
...
def test_excised_entries_do_not_trigger_the_abort():
    ...
    derive(s, geo, bg, EOS, lay)  # no exception
