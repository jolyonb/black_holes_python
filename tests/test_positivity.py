"""Property tests of the positivity lemma for the energy row (Section 7.7), written test-first.
...
interior faces' flux specification. `check_pair_n` pins the rest of face N (Section 7.5): face N has ONE state,
...
chord everywhere beyond it. The tests were written before the scheme had the theta-limiter and the chord-widened
bounds, and failed on the scheme of the time where the lemma says it must fail; each ingredient's tests went green
with the bite that brought it.
...
#: `rho - 1`, so a face value carries an absolute error of a few `eps`. Measured worst (both w, theta 0.1 and 0.2):
#: about 1.7, so 8 leaves a margin of about 5. In absolute terms the tolerance
...
#: error of a few `eps`, which is `eps / rho_c` relative. Measured worst at `theta = THETA_MIN`:
...
#: `face_scale`). Measured worst over every family, seed and `w`: 0.25 eps for the gain and 19.7 eps for
...
#: (Section 7.2), and a gain that is zero to round-off of the dense neighbours' terms is `eps cell_scale`, which
#: over the step can exceed `1e-6` of a content at `5e-13` (measured: 9.6e-6 of the content, which is
...
class Audit:
    ...
    Attributes:
        ...
        E: The contents.
        dV: The cell volumes: the content of a cell is known only to about `eps dV` (Section 7.2).
...
def audit(sch: Scheme, case: Case) -> Audit:
    """Evaluate the scheme on the case and recompute the lemma's quantities from its outputs, with the bounds (B)."""
    lay = sch.layout
    N, j_e = lay.N, lay.j_e
    eos = case.eos
    ...
    k = res.kernels
    if k is None:  # not an assert: a precondition of the test, not a claim of the lemma
...
def check_pair_n(sch: Scheme, case: Case) -> list[str]:
    """Face N has one state (Section 7.5): the pair `(rho_hat_N, e^phi(rho_hat_N))`, `rho_hat_N` the last cell's
    ...
    It is not the extrapolation `<f>_N = 3/2 f_(N-1) - 1/2 f_(N-2)` of the density and of the lapse, which can turn
    non-positive (Section 7.3). Tolerances: the pair is known to an absolute `eps (1 + rho_hat)`, so
    each comparison allows the change `PAIR_ROUNDOFF_EPS` such `eps` in the density make, plus that many `eps` of the
    quantity's own size. The SAT closure only: the held face of the pinned map is outside the lemma (Section 7.7).
    """
    if case.closure != "sat":
        return []
    N = sch.layout.N
...
def face_n_value_kernels_off(sch: Scheme, case: Case, d: Derived) -> tuple[float, float]:
    ...
    evaluated at face N (Section 7.5: the face state is defined without the kernels)."""
    N = case.layout.N
    geo = sch.frame(case.xi).geo
...
def check_pair_n_kernels_off(sch: Scheme, case: Case) -> list[str]:
    ...
    if res.kernels is not None:  # not an assert: a precondition of the test, not a claim of the lemma
...
# --- the generator, and the excision face ---
...
def test_the_generated_states_are_violent_and_admissible(w: str, family: str):
    ...
    if family == "excised":
        ...
        for seed in SEEDS:
            ...
            acoustic_open += int(max(Theta_plus_a, 0.0) > 0.0)  # Lambda+ > 0 at the face from the acoustic speed alone
        # the excision rule rho^L := rho^R is visible to a flux only where Lambda+ > 0 at j_e: both kinds must occur
        assert chord_out >= 1
        assert acoustic_open >= 1
...
def test_the_excision_face_carries_only_the_first_cells_own_content(w: str):
    ...
    # (with Lambda+ = 0 the HLL flux is F(rho^R) whatever rho^L is): the open seeds, where the acoustic speed alone
    # makes it positive, and the trapped face whose chord points outward (the generator asserts both occur). Setting
    # rho^L_je to the cell mean makes this test fail on the open seeds (Section 8.3).
    for seed in SEEDS:
...
def test_face_n_has_one_state_the_pair_of_its_reconstructed_value(w: str, family: str):
    ...
    # e^phi(rho_hat_N)) that F_N carries (Section 7.5), never the extrapolations, which can turn negative.
...
def test_with_the_kernels_off_face_n_has_one_state_the_pair_of_the_last_cells_slope(w: str, family: str):
    ...
    # here from the theta-limited one-sided s-slope of cells N-2 and N-1 (Section 7.5).
