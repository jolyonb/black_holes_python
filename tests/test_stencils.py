...
def weights(geo: Geometry, j_e: int = 0) -> StencilWeights:
    return StencilWeights.of(geo, Layout(geo.N, j_e))
...
def test_the_gradient_in_s_is_exact_on_a_field_linear_in_s(family: Family):
    geo = geometry(family, 12, 3.0)
    a, b = 0.7, -0.4
    f = a + b * geo.sbar[:-1]  # the shell average of a + b X^2 is its value at sbar, exactly
    for j_e in (0, 3):
        grad = weights(geo, j_e).gradient_s(f)
        expected = 2.0 * b * geo.X  # d/dX (a + b X^2)
        assert grad[j_e + 1 : 12] == pytest.approx(expected[j_e + 1 : 12], rel=1e-12)
        assert grad[j_e] == 0.0  # none at the origin, none formed at an excision face
        assert math.isnan(grad[12])


@pytest.mark.parametrize("family", FAMILIES)
def test_the_face_value_at_an_excision_face_is_the_cell_behind_it(family: Family):
    geo = geometry(family, 12, 3.0)
    f = 0.7 - 0.4 * geo.sbar[:-1]
    assert weights(geo, 3).face_average(f)[3] == f[3]


@pytest.mark.parametrize("family", FAMILIES)
def test_every_velocity_row_gives_one_on_u_equals_x(family: Family):
    # (D_U X)_j = 1 exactly is what makes FRW exact (Section 7.3), at every row including the one-sided ones.
    geo = geometry(family, 12, 3.0)
    for j_e in (0, 3):
        grad = weights(geo, j_e).velocity_gradient(geo.X)
        assert grad[max(j_e, 1) :] == pytest.approx(1.0, rel=1e-12)
        assert math.isnan(grad[0])


@pytest.mark.parametrize("family", FAMILIES)
def test_the_outer_row_is_exact_on_a_quadratic_and_the_excision_row_is_one_difference(family: Family):
    geo = geometry(family, 12, 3.0)
    X = geo.X
    U = 0.3 - 1.1 * X + 0.8 * X**2
    dU = -1.1 + 1.6 * X
    grad = weights(geo, 3).velocity_gradient(U)
    assert grad[12] == pytest.approx(dU[12], rel=1e-12)  # the three-point outer row
    assert grad[3] == pytest.approx((U[4] - U[3]) / geo.dX[3], rel=1e-15)  # one difference at j_e ...
    assert grad[3] != pytest.approx(dU[3], rel=1e-6)  # ... first order, not exact on a quadratic
