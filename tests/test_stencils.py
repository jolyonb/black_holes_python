...
MAPS: list[Map] = [IdentityMap(), SinhStretch(scale=1.5)]


def geometry(m: Map, N: int, x_max: float) -> Geometry:
    return Geometry.of(*m.at(0.0, face_labels(N, x_max)))
...
def test_the_gradient_in_s_is_exact_on_a_field_linear_in_s(m: Map, closure: FaceClosure):
    geo = geometry(m, 12, 3.0)
    a, b = 0.7, -0.4
...
@pytest.mark.parametrize("m", MAPS)
def test_the_second_order_face_value_is_exact_on_a_field_linear_in_s_and_the_first_order_one_is_the_cell(m: Map):
    geo = geometry(m, 12, 3.0)
    a, b = 0.7, -0.4
    f = a + b * geo.sbar[:-1]
...
@pytest.mark.parametrize("m", MAPS)
@pytest.mark.parametrize("closure", [O1, O2])
def test_every_velocity_row_gives_one_on_u_equals_x(m: Map, closure: FaceClosure):
...
@pytest.mark.parametrize("m", MAPS)
def test_the_three_point_rows_are_exact_on_a_quadratic_and_the_first_order_row_is_not(m: Map):
    geo = geometry(m, 12, 3.0)
    X = geo.X
    U = 0.3 - 1.1 * X + 0.8 * X**2
    dU = -1.1 + 1.6 * X
...
def test_the_gradient_in_s_is_second_order_down_to_the_first_face(m: Map, face: int):
    e_s = [face_errors(m, N, face)[0] for N in (20, 40, 80)]
...
def test_the_gradient_in_x_is_only_first_order_at_the_first_faces(m: Map, face: int):
    ...
    e_X = [face_errors(m, N, face)[1] for N in (20, 40, 80)]
